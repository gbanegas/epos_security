...
void MIPS32::init()
{
    ...
    //Physical memory
    si->pmm.mem_base    = Memory_Map<PLASMA>::BASE;
    si->pmm.mem_top     = Memory_Map<PLASMA>::TOP;
    si->pmm.io_mem_base = Memory_Map<PLASMA>::IO_MEM;
    si->pmm.io_mem_top  = Memory_Map<PLASMA>::IO_MEM + 0x00010000;
    si->pmm.int_vec     = Memory_Map<PLASMA>::INT_VEC;
    si->pmm.sys_code    = Memory_Map<PLASMA>::SYS_CODE;
    si->pmm.sys_data    = Memory_Map<PLASMA>::SYS_DATA;
    si->pmm.sys_stack   = Memory_Map<PLASMA>::SYS_STACK;
    si->pmm.free_base   = 0x100AA000; //0x10020000; //////////(unsigned)&__bss_end;
    si->pmm.free_top    = Memory_Map<Machine>::TOP - Traits<PLASMA>::SYSTEM_STACK_SIZE;
